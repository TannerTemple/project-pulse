<template>
  <v-container class="pa-6" fluid>
    ...
    <v-row v-if="auth.role === 'ADMIN'" class="mt-2">
      <v-col v-for="card in adminCards" :key="card.title" cols="12" sm="6" md="4" lg="3">
        <v-card rounded="lg" hover :to="card.to">
    ...
    <v-row v-else-if="auth.role === 'INSTRUCTOR'" class="mt-2">
      <v-col v-for="card in instructorCards" :key="card.title" cols="12" sm="6" md="4">
        <v-card rounded="lg" hover :to="card.to">
          <v-card-text class="d-flex align-center ga-4 pa-5">
            <v-avatar :color="card.color" size="48" rounded="lg">
              <v-icon :icon="card.icon" color="white" />
    ...
    <v-row v-else class="mt-2">
      <v-col v-for="card in studentCards" :key="card.title" cols="12" sm="6" md="4">
        <v-card rounded="lg" hover :to="card.to">
          <v-card-text class="d-flex align-center ga-4 pa-5">
            <v-avatar :color="card.color" size="48" rounded="lg">
              <v-icon :icon="card.icon" color="white" />
